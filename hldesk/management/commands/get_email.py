...
scripts/get_email.py - Designed to be run from cron, this script checks the
                       ...
                       hldesk, creating tickets from the new messages (or
...
from hldesk.lib import send_templated_mail, safe_template_context, process_attachments
from hldesk.models import Queue, Ticket, TicketCC, FollowUp, IgnoreEmail
...
class Command(BaseCommand):
    ...
    help = 'Process django-hldesk queues and process e-mails via POP3/IMAP or ' \
           'from a local mailbox directory as required, feeding them into the hldesk.'
...
def process_email(quiet=False):
    for q in Queue.objects.filter(
        ...
        logger = logging.getLogger('django.hldesk.queue.' + q.slug)
        ...
        logdir = q.logging_dir or '/var/log/hldesk/'
...
def process_queue(q, logger):
    logger.info("***** %s: Begin processing mail for django-hldesk" % ctime())
    ...
    if email_box_type == 'pop3':
        ...
        mail_dir = q.email_box_local_dir or '/var/lib/mail/hldesk/'

...
from hldesk.models import Attachment, EmailTemplate

logger = logging.getLogger('hldesk')
...
def send_templated_mail(template_name,
    ...
    from hldesk.models import EmailTemplate
    from hldesk.settings import HELPDESK_EMAIL_SUBJECT_TEMPLATE, \
    ...
    footer_file = os.path.join('hldesk', locale, 'email_text_footer.txt')

    text_part = from_string(
        "%s{%% include '%s' %%}" % (t.plain_text, footer_file)
    ).render(context)

    email_html_base_file = os.path.join('hldesk', locale, 'email_html_base.html')
...
def text_is_spam(text, request):
    ...
    try:
        from hldesk.akismet import Akismet
    except ImportError:
        return False
    try:

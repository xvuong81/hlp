...
from hldesk.models import CustomField, Queue, Ticket
...
from hldesk.templatetags.ticket_to_link import num_to_link
from hldesk.views.staff import _is_my_ticket
...
class TicketActionsTestCase(TestCase):
    ...
    def test_delete_ticket_staff(self):
        ...
        response = self.client.get(reverse('hldesk:delete', kwargs={'ticket_id': ticket_id}), follow=True)
        self.assertContains(response, 'Are you sure you want to delete this ticket')

        response = self.client.post(reverse('hldesk:delete', kwargs={'ticket_id': ticket_id}), follow=True)
        ...
        self.assertEqual(urlparts.path, reverse('hldesk:home'))
    ...
    def test_update_ticket_staff(self):
        ...
        response = self.client.post(reverse('hldesk:update', kwargs={'ticket_id': ticket_id}), post_data, follow=True)
    ...
    def test_num_to_link(self):
        ...
        self.assertEqual(result, "this is ticket <a href='/hldesk/tickets/%s/' class='ticket_link_status ticket_link_status_Open'>#%s</a>" % (ticket_id, ticket_id))

        result2 = num_to_link('whoa another ticket is here #%s huh' % ticket_id)
        self.assertEqual(result2, "whoa another ticket is here  <a href='/hldesk/tickets/%s/' class='ticket_link_status ticket_link_status_Open'>#%s</a> huh" % (ticket_id, ticket_id))

    def test_create_ticket_getform(self):
        self.loginUser()
        response = self.client.get(reverse('hldesk:submit'), follow=True)

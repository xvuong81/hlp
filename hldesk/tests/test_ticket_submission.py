from hldesk.models import Queue, CustomField, Ticket
...
class TicketBasicsTestCase(TestCase):
    ...
    def test_create_ticket_public(self):
        ...
        response = self.client.post(reverse('hldesk:home'), post_data, follow=True)
        ...
        self.assertEqual(urlparts.path, reverse('hldesk:public_view'))
    ...
    def test_create_ticket_private(self):
        ...
        response = self.client.post(reverse('hldesk:home'), post_data)
        self.assertEqual(response.status_code, 200)
        self.assertEqual(email_count, len(mail.outbox))

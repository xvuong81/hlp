...
from hldesk.models import Queue, Ticket, TicketCC, FollowUp, Attachment
...
class GetEmailCommonTests(TestCase):
    ...
    def test_get_email_quiet_option(self):
        ...
        with mock.patch('hldesk.management.commands.get_email.process_email') as mocked_processemail:
...
class GetEmailParametricTemplate(object):
    ...
    def setUp(self):
        ...
        if self.method == 'local':
            kwargs["email_box_local_dir"] = '/var/lib/mail/hldesk/'
        else:
    ...
    def test_read_plain_email(self):
        ...
        test_email_body = "Unicode hldesk comment with an s-hat (ŝ) via email."
        test_email = "To: hldesk@example.com\nFrom: " + test_email_from + "\nSubject: " + test_email_subject + "\n\n" + test_email_body
        ...
        if self.socks:
            ...
            if self.method == 'local':
                with mock.patch('hldesk.management.commands.get_email.listdir') as mocked_listdir, \
                        mock.patch('hldesk.management.commands.get_email.isfile') as mocked_isfile, \
                    ...
                    mocked_listdir.assert_called_with('/var/lib/mail/hldesk/')
                    mocked_isfile.assert_any_call('/var/lib/mail/hldesk/filename1')
                    mocked_isfile.assert_any_call('/var/lib/mail/hldesk/filename2')
                ...
                with mock.patch('hldesk.management.commands.get_email.poplib', autospec=True) as mocked_poplib:
                ...
                with mock.patch('hldesk.management.commands.get_email.imaplib', autospec=True) as mocked_imaplib:
    ...
    def test_read_email_with_template_tag(self):
        ...
        test_email_from = "Arnbjörg Ráðormsdóttir <arnbjorg@example.com>"
        test_email_subject = "My visit to Sør-Trøndelag"
        test_email_body = "Reporting some issue with the template tag: {% if hldesk %}."
        test_email = "To: hldesk@example.com\nFrom: " + test_email_from + "\nSubject: " + test_email_subject + "\n\n" + test_email_body
        test_mail_len = len(test_email)
    ...
    def test_read_pgp_signed_email(self):
        ...
        test_email = """Delivered-To: djangohldesk@example.com
...
Received: by mail-pf0-x233.google.com with SMTP id p87so2672006pfj.3
        for <djangohldesk@example.com>; Wed, 08 Nov 2017 13:30:21 -0800 (PST)
...
To: djangohldesk@example.com
From: Bug Reporter <bugreporter@example.com>
Subject: example email that crashes django-hldesk get_email
...
https://github.com/django-hldesk/django-hldesk/issues/567#issuecommen=
t-342954233
...
"""
        ...
        if self.socks:
            ...
            ticket1 = get_object_or_404(Ticket, pk=1)
            self.assertEqual(ticket1.ticket_for_url, "QQ-%s" % ticket1.id)
            self.assertEqual(ticket1.title, "example email that crashes django-hldesk get_email")
            self.assertEqual(ticket1.description, """hi, thanks for looking into this :)\n\nhttps://github.com/django-hldesk/django-hldesk/issues/567#issuecomment-342954233""")
...
class GetEmailCCHandling(TestCase):
    ...
    def setUp(self):
        ...
        kwargs = {
            ...
            "email_box_local_dir": '/var/lib/mail/hldesk/',
            "logging_dir": self.temp_logdir,
            "logging_type": 'none'
        }
        self.queue_public = Queue.objects.create(**kwargs)

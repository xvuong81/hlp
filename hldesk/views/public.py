...
from hldesk.decorators import protect_view
from hldesk.forms import PublicTicketForm
from hldesk.lib import text_is_spam
from hldesk.models import Ticket, Queue, KBCategory
...
def homepage(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            if text_is_spam(form.cleaned_data['body'], request):
                ...
                return render(request, template_name='hldesk/public_spam.html')
            else:
                ticket = form.save(
                    user=request.user if request.user.is_authenticated else None)
                try:
                    return HttpResponseRedirect('%s?ticket=%s&email=%s' % (
                        reverse('hldesk:public_view'),
    ...
    return render(request, 'hldesk/public_homepage.html', {
        'form': form,
...
def view_ticket(request):
    ...
    if ticket_req and email:
        ...
        try:
            ...
            if request.user.is_staff:
                redirect_url = reverse('hldesk:view', args=[ticket_id])
                if 'close' in request.GET:
                    redirect_url += '?close'
                return HttpResponseRedirect(redirect_url)

            if 'close' in request.GET and ticket.status == Ticket.RESOLVED_STATUS:
                from hldesk.views.staff import update_ticket
            ...
            redirect_url = ''
            if hldesk_settings.HELPDESK_NAVIGATION_ENABLED:
                redirect_url = reverse('hldesk:view', args=[ticket_id])

            return render(request, 'hldesk/public_view_ticket.html', {
                'ticket': ticket,
                'hldesk_settings': hldesk_settings,
                'next': redirect_url,
            })
    ...
    return render(request, 'hldesk/public_view_form.html', {
        'ticket': False,
        'email': email,
...
def change_language(request):
    ...
    return render(request, 'hldesk/public_change_language.html', {'next': return_to})

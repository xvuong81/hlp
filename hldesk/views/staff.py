...
def _get_user_queues(user):
    ...
    limit_queues_by_user = \
        hldesk_settings.HELPDESK_ENABLE_PER_QUEUE_STAFF_PERMISSION \
...
def _has_access_to_queue(user, queue):
    ...
    :param user: The User (the class should have the has_perm method)
    :param queue: The django-hldesk Queue instance
    :return: True if the user has permission (either by default or explicitly), false otherwise
    """
    if user.is_superuser or not hldesk_settings.HELPDESK_ENABLE_PER_QUEUE_STAFF_PERMISSION:
        return True
    else:
...
def dashboard(request):
    ...
    from_clause = """FROM    hldesk_ticket t,
                    hldesk_queue q"""
    if queues:
    ...
    return render(request, 'hldesk/dashboard.html', {
...
def delete_ticket(request, ticket_id):
    ...
    if request.method == 'GET':
        return render(request, 'hldesk/delete_ticket.html', {
            'ticket': ticket,
        })
    else:
        ticket.delete()
        return HttpResponseRedirect(reverse('hldesk:home'))
...
def followup_edit(request, ticket_id, followup_id):
    ...
    if request.method == 'GET':
        ...
        return render(request, 'hldesk/followup_edit.html', {
            'followup': followup,
            'ticket': ticket,
            'form': form,
        ...
        if form.is_valid():
            ...
            # delete old followup
            followup.delete()
        return HttpResponseRedirect(reverse('hldesk:view', args=[ticket.id]))
...
def view_ticket(request, ticket_id):
    ...
    if hldesk_settings.HELPDESK_STAFF_ONLY_TICKET_OWNERS:
    ...
    return render(request, 'hldesk/ticket.html', {
        'ticket': ticket,
        'form': form,
        'active_users': users,
...
def update_ticket(request, ticket_id, public=False):
    if not (public or (
            ...
            request.user.is_active and (
                ...
                hldesk_settings.HELPDESK_ALLOW_NON_STAFF_TICKET_UPDATE))):
        return HttpResponseRedirect('%s?next=%s' %
                                    (reverse('hldesk:login'), request.path))
    ...
    if no_changes:
        return return_to_ticket(request.user, hldesk_settings, ticket)
    ...
    if request.user.is_staff or hldesk_settings.HELPDESK_ALLOW_NON_STAFF_TICKET_UPDATE:
        f.user = request.user
    ...
    if ticket.assigned_to and \
        ...
        if (not reassigned or
                (reassigned and
                    ticket.assigned_to.usersettings_hldesk.settings.get(
    ...
    if hldesk_settings.HELPDESK_AUTO_SUBSCRIBE_ON_TICKET_RESPONSE and request.user.is_authenticated:
...
def return_to_ticket(user, hldesk_settings, ticket):
    """Helper function for update_ticket"""

    if user.is_staff or hldesk_settings.HELPDESK_ALLOW_NON_STAFF_TICKET_UPDATE:
...
def mass_update(request):
    ...
    if not (tickets and action):
        return HttpResponseRedirect(reverse('hldesk:list'))
...
def ticket_list(request):
    ...
    if request.GET.get('saved_query', None):
        ...
        import json
        from hldesk.lib import b64decode
        try:
            if six.PY3:
    ...
    import json
    from hldesk.lib import b64encode
    urlsafe_query = b64encode(json.dumps(query_params).encode('UTF-8'))

    user_saved_queries = SavedSearch.objects.filter(Q(user=request.user) | Q(shared__exact=True))

    return render(request, 'hldesk/ticket_list.html', dict(
        context,
        tickets=ticket_qs,
...
def edit_ticket(request, ticket_id):
    ...
    return render(request, 'hldesk/edit_ticket.html', {'form': form})
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if _has_access_to_queue(request.user, ticket.queue):
                ...
                return HttpResponseRedirect(reverse('hldesk:dashboard'))
    else:
        initial_data = {}
        ...
        form.fields['assigned_to'].choices = [('', '--------')] + [
            (u.id, u.get_username()) for u in assignable_users]
        if hldesk_settings.HELPDESK_CREATE_TICKET_HIDE_ASSIGNED_TO:
            form.fields['assigned_to'].widget = forms.HiddenInput()

    return render(request, 'hldesk/create_ticket.html', {'form': form})
...
def rss_list(request):
    return render(request, 'hldesk/rss_list.html', {'queues': Queue.objects.all()})
...
def report_index(request):
    ...
    return render(request, 'hldesk/report_index.html', {
...
def run_report(request, report):
    if Ticket.objects.all().count() == 0 or report not in (
        ...
        return HttpResponseRedirect(reverse("hldesk:report_index"))
    ...
    return render(request, 'hldesk/report_output.html', {
        'title': title,
        'charttype': charttype,
        'data': table,
...
def save_query(request):
    ...
    return HttpResponseRedirect('%s?saved_query=%s' % (reverse('hldesk:list'), query.id))
...
def delete_saved_query(request, id):
    ...
    if request.method == 'POST':
        ...
        return render(request, 'hldesk/confirm_delete_saved_query.html', {'query': query})


@staff_member_required
def user_settings(request):
    s = request.user.usersettings_hldesk
    if request.POST:
    ...
    return render(request, 'hldesk/user_settings.html', {'form': form})


@superuser_required
def email_ignore(request):
    return render(request, 'hldesk/email_ignore_list.html', {
...
def email_ignore_add(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return HttpResponseRedirect(reverse('hldesk:email_ignore'))
    else:
        form = EmailIgnoreForm(request.GET)

    return render(request, 'hldesk/email_ignore_add.html', {'form': form})
...
def email_ignore_del(request, id):
    ...
    if request.method == 'POST':
        ...
        return render(request, 'hldesk/email_ignore_del.html', {'ignore': ignore})
...
def ticket_cc(request, ticket_id):
    ...
    return render(request, 'hldesk/ticket_cc_list.html', {
        'copies_to': copies_to,
        'ticket': ticket,
    })
...
def ticket_cc_add(request, ticket_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            ticketcc.save()
            return HttpResponseRedirect(reverse('hldesk:ticket_cc',
                                                kwargs={'ticket_id': ticket.id}))
    else:
        form_email = TicketCCEmailForm()
        form_user = TicketCCUserForm()
    return render(request, 'hldesk/ticket_cc_add.html', {
        'ticket': ticket,
...
def ticket_cc_del(request, ticket_id, cc_id):
    ...
    return render(request, 'hldesk/ticket_cc_del.html', {'cc': cc})
...
def ticket_dependency_add(request, ticket_id):
    ...
    return render(request, 'hldesk/ticket_dependency_add.html', {
        'ticket': ticket,
        'form': form,
    })
...
def ticket_dependency_del(request, ticket_id, dependency_id):
    ...
    if request.method == 'POST':
        dependency.delete()
        return HttpResponseRedirect(reverse('hldesk:view', args=[ticket_id]))
    return render(request, 'hldesk/ticket_dependency_del.html', {'dependency': dependency})
...
def attachment_del(request, ticket_id, attachment_id):
    ...
    if request.method == 'POST':
        attachment.delete()
        return HttpResponseRedirect(reverse('hldesk:view', args=[ticket_id]))
    return render(request, 'hldesk/ticket_attachment_del.html', {
